  ...
  const [history, setHistory] = useState<Message[]>([
    {
      role: "assistant",
      content:
        "Hello! Ask me anything about the engineering programs in Manipal !!",
    },
  ]);
  ...
  return (
    <main className="h-screen bg-white p-6 flex flex-col">
      <div className="flex flex-col gap-8 w-full items-center flex-grow max-h-full">
        <h1 className=" text-4xl text-transparent font-extralight bg-clip-text bg-gradient-to-r from-orange-800 to-fuchsia-500">
          Program Helper Chatbot
        </h1>
        <form
          className="rounded-2xl border-orange-700 border-opacity-5  border lg:w-3/4 flex-grow flex flex-col bg-[url('/images/bg.png')] bg-cover max-h-full overflow-clip"
          onSubmit={(e) => {
            e.preventDefault();
            handleClick();
          }}
                            ...
                            {message.links?.map((link) => {
                              return (
                                <a
                                  href={link}
                                  key={link}
                                  className="block w-fit px-2 py-1 text-sm  text-orange-700 bg-orange-100 rounded"
                                >
                                  {formatPageName(link)}
                                </a>
                              );
          ...
          <div className="flex sticky bottom-0 w-full px-6 pb-6 h-24">
            <div className="w-full relative">
              <textarea
                ...
                className="w-full h-full resize-none rounded-full border border-slate-900/10 bg-white pl-6 pr-24 py-[25px] text-base placeholder:text-slate-400 focus:border-orange-500 focus:outline-none focus:ring-4 focus:ring-orange-500/10 shadow-[0_10px_40px_0px_rgba(0,0,0,0.15)]"
                onKeyDown={(e) => {
              ...
              <button
                ...
                className="flex w-14 h-14 items-center justify-center rounded-full px-3 text-sm  bg-orange-600 font-semibold text-white hover:bg-orange-700 active:bg-orange-800 absolute right-2 bottom-2 disabled:bg-orange-100 disabled:text-orange-400"
                type="submit"
                aria-label="Send"
